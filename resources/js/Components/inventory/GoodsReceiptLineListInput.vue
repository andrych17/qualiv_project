<!-- ponytail: repeatable Goods Receipt lines (§3D) -->
...
export interface ReceiptLineRow {
  ...
  batch_manufacture_date?: string
  batch_supplier_reference?: string
  serial_numbers?: string[]
}

const props = defineProps<{
  modelValue: ReceiptLineRow[]
  uoms: Array<{ id: number; code: string; name: string }>
  locations: Array<{ id: number; warehouse_id: number; code: string }>
  warehouseId: number | null
  /** §3L: product_id → tracking_mode, so a batch-tracked line shows the lot fields. */
...
/** §3M: a serial-tracked line names its units instead of typing a quantity — qty is derived. */
const isSerialTracked = (productId: number | null) => !!productId && props.productTracking?.[productId] === 'serial'

const emit = defineEmits<{
  'update:modelValue': [value: ReceiptLineRow[]]
}>()
...
const emptyRow = (): ReceiptLineRow => ({ product_id: null, qty: 1, uom_id: null, unit_cost: 0, destination_location_id: null })
...
const update = (index: number, patch: Partial<ReceiptLineRow>) => {
...
// §3K scan-to-receive: rescanning a product already on a line bumps its qty (a case-pack
// barcode bumps by its unit_multiplier) instead of adding a duplicate row.
...
const onScanned = (scanned: ProductScanResult | LocationScanResult) => {
  ...
  if (existingIndex >= 0) {
    ...
    emit('update:modelValue', [
      ...props.modelValue,
      { product_id: result.product_id, qty: result.unit_multiplier, uom_id: result.uom_id, unit_cost: 0, destination_location_id: null },
    ])
  }
}
...
<template>
  <div class="space-y-3">
    ...
    >
      <div class="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] items-end gap-3">
        <FormAsyncSearchableSelect
          ...
          :disabled="disabled"
          @update:model-value="update(index, { product_id: Number($event) })"
        />
        ...
        <div class="space-y-1.5">
          <label class="text-sm font-medium text-ink-900">Unit cost</label>
          <input
            :value="row.unit_cost"
            type="number"
            step="0.000001"
            ...
            :disabled="disabled"
            class="w-full rounded-sm border border-border bg-surface-0 px-3 py-2 text-sm text-ink-900 shadow-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:bg-surface-50 disabled:text-ink-600"
            @input="update(index, { unit_cost: Number(($event.target as HTMLInputElement).value) })"
          />
        </div>

        <FormSelect
          :model-value="row.destination_location_id"
          :name="`lines.${index}.destination_location_id`"
          label="Destination"
          placeholder="Choose a bin…"
          :options="locationOptions"
          :disabled="disabled || !warehouseId"
          @update:model-value="update(index, { destination_location_id: Number($event) })"
        />
      ...
      <div v-if="isBatchTracked(row.product_id)" class="grid grid-cols-4 items-end gap-3 rounded-sm bg-surface-50 p-2.5">
        <div class="space-y-1.5">
          <label class="text-sm font-medium text-ink-900">Lot number</label>
          <input
            :value="row.batch_number"
            placeholder="Lot / batch #"
            :disabled="disabled"
            class="w-full rounded-sm border border-border bg-surface-0 px-3 py-2 text-sm text-ink-900 shadow-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:bg-surface-50 disabled:text-ink-600"
            @input="update(index, { batch_number: ($event.target as HTMLInputElement).value })"
          />
        </div>
        <div class="space-y-1.5">
          <label class="text-sm font-medium text-ink-900">Manufactured</label>
          <input
            :value="row.batch_manufacture_date"
            type="date"
            :disabled="disabled"
            class="w-full rounded-sm border border-border bg-surface-0 px-3 py-2 text-sm text-ink-900 shadow-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:bg-surface-50 disabled:text-ink-600"
            @input="update(index, { batch_manufacture_date: ($event.target as HTMLInputElement).value })"
          />
        </div>
        <div class="space-y-1.5">
          <label class="text-sm font-medium text-ink-900">Expiry</label>
          <input
            :value="row.batch_expiry_date"
            type="date"
            :disabled="disabled"
            class="w-full rounded-sm border border-border bg-surface-0 px-3 py-2 text-sm text-ink-900 shadow-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:bg-surface-50 disabled:text-ink-600"
            @input="update(index, { batch_expiry_date: ($event.target as HTMLInputElement).value })"
          />
        </div>
        <div class="space-y-1.5">
          <label class="text-sm font-medium text-ink-900">Supplier ref</label>
          <input
            :value="row.batch_supplier_reference"
            :disabled="disabled"
            class="w-full rounded-sm border border-border bg-surface-0 px-3 py-2 text-sm text-ink-900 shadow-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:bg-surface-50 disabled:text-ink-600"
            @input="update(index, { batch_supplier_reference: ($event.target as HTMLInputElement).value })"
          />
        </div>
      </div>
