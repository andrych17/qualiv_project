...
const destroy = (t: TemplateRow) => {
  if (confirm(`Delete recurring template "${t.name}"? This does not affect invoices already generated.`)) {
    router.delete(route('accounting.recurring-ar-templates.destroy', t.id))
  }
}
</script>
...
<template>
  <AppLayout>
    <PageHeader title="Recurring invoices" description="An invoice draft is generated automatically each time the rule comes due — always a draft, never posted, until reviewed.">
      <template #actions>
        <PrimaryButton :href="route('accounting.recurring-ar-templates.create', { company_id: selectedCompanyId })">New template</PrimaryButton>
      </template>
    </PageHeader>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="t in templates" :key="t.id" class="border-b border-border hover:bg-surface-50">
            <td class="py-2">
              <a :href="route('accounting.recurring-ar-templates.edit', t.id)" class="font-medium text-accent hover:underline">{{ t.name }}</a>
            </td>
            <td class="py-2 text-ink-700">{{ t.partner_name ?? '—' }}</td>
          ...
          <tr v-if="!templates.length"><td colspan="7" class="py-6 text-center text-ink-600">No recurring invoice templates yet.</td></tr>
        </tbody>
      </table>
    </Panel>
  </AppLayout>
