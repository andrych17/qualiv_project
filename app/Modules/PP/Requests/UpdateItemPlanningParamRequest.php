...
class UpdateItemPlanningParamRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            $currentId = $this->route('item_planning_param')?->id;

            if ($productId && ! Product::query()->whereKey($productId)->exists()) {
                $validator->errors()->add('product_id', 'The selected product is invalid.');
            }

            if ($productId && ItemPlanningParam::query()->where('product_id', $productId)->where('id', '!=', $currentId)->exists()) {
