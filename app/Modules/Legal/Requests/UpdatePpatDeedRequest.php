...
class UpdatePpatDeedRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'land_object_id' => 'required|integer',
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            if ($deedTypeId && (! $deedType || $deedType->category !== DeedType::CATEGORY_PPAT)) {
                $validator->errors()->add('deed_type_id', 'The selected deed type is invalid.');
            }

            $landObjectId = $this->input('land_object_id');
            if ($landObjectId && ! LandObject::query()->whereKey($landObjectId)->exists()) {
                $validator->errors()->add('land_object_id', 'The selected land object is invalid.');
            }
        });
    }
}
